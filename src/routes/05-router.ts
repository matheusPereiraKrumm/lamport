...
export class Server5 {
    ...
    private start(request:Request, response:Response){
        ...
        Server5.ExecutaProximo();

        Server5.envia(response, {})
    }

    private static ExecutaProximo(){
        var acaoAtual:number = execucao.pop();
        if(acaoAtual){
            Server5.PedePermissaoDeAcesso();
            
            Server5.ValidaPermissaoUso(acaoAtual);
        }
    }
    ...
    private static ValidaPermissaoUso(acaoAtual:number){
        if(filaOk.length == filaServers.length){
            Server5.ExecutaAcao(acaoAtual);
        }else{
            setTimeout(() => {Server5.ValidaPermissaoUso(acaoAtual)},100);
        }
    }    
    ...
    private acesso(request:Request, response:Response){
        Server5.AtualizaTempo(request.body.time);
        if(Esperando){
            if(TimeInterno == request.body.time){
                if(myRoute < request.body.pdi){
                    Server5.envia(response, {liberado:false, time: Time, pdi: myRoute})
                }else {
                    if(Executando){
                        Server5.envia(response, {liberado:false, time: Time, pdi: myRoute})
                    }else Server5.envia(response, {liberado:true, time: Time, pdi: myRoute})
                }
                ...
                if(Executando){
                    ...
                    Server5.envia(response, {liberado:true, time: Time, pdi: myRoute})
                }
            }
        }else{
            TimeInterno = Time;
    ...
    private libera(request:Request, response:Response){
        ...
        Server5.envia(response, {time: Time, pdi: myRoute})
    }
}
...
const myRoute:number = 5;

const ser = new Server5();
ser.init();
